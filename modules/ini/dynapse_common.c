...
static uint32_t convertBias(const char *biasName, const char* lowhi,
                            const char*cl, const char*sex, uint8_t enal,  uint8_t fineValue, uint8_t coarseValue, uint8_t special);
...
// Chip biases, defaults.
    ...
    createCoarseFineBiasSetting(biasNode, "U_Buffer", 1, 80, "HighBias",
                             "Normal", "PBias", true);
    createCoarseFineBiasSetting(biasNode, "U_SSP", 0, 7,"HighBias",
                             "Normal", "PBias", true);
    createCoarseFineBiasSetting(biasNode, "U_SSN", 0, 15,"HighBias",
                             "Normal", "PBias", true);

    createCoarseFineBiasSetting(biasNode, "D_Buffer", 1, 80, "HighBias",
                             "Normal", "PBias", true);
    createCoarseFineBiasSetting(biasNode, "D_SSP", 0, 7, "HighBias",
                             "Normal", "PBias", true);
    createCoarseFineBiasSetting(biasNode, "D_SSN", 0, 15, "HighBias",
                             "Normal", "PBias", true);
...
static uint32_t convertBias(const char *biasName, const char* lowhi,
		...
		if(addr == 51 || addr == 52 || addr == 115 || addr == 116){
			...
			inbits = addr << 18 | 1 << 16 | special << 15 | coarseValue << 12 | fineValue << 4 | confbits;
		}
...
static void biasConfigSend(sshsNode node, caerModuleData moduleData,
  ...
  //1110100011111110000001101
    ...
    for(uint8_t this_chip = 4; this_chip <= 4; this_chip++){
        ...
        if (biasNodes != NULL) {
            for (size_t i = 0; i < biasNodesLength; i++) {
                ...
                uint8_t coarseValue = sshsNodeGetShort(biasNodes[i], "coarseValue");
                uint8_t fineValue = sshsNodeGetShort(biasNodes[i], "fineValue");
                const char * lowhi = sshsNodeGetString(biasNodes[i], "BiasLowHi");
                const char * cl = sshsNodeGetString(biasNodes[i], "currentLevel");
                const char * sex = sshsNodeGetString(biasNodes[i], "sex");

                // generates bits values
                uint32_t bits = convertBias(nodeName, lowhi, cl, sex, enal, fineValue, coarseValue, special);
                
                caerLog(CAER_LOG_DEBUG, moduleData->moduleSubSystemString, "BIAS LENGHT ... %d NAME %s enabled %d special %d coarseValue %d bits %d\n",
                        biasNodesLength, nodeName, enal, special, coarseValue, bits);
