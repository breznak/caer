/*
 * Created on: Dec, 2016
 * Author: dongchen@ini.uzh.ch, federico.corradi@inilabs.com
 */
...
typedef struct LFilter_memory LFilterMemory;
...
static void caerLearningFilterRun(caerModuleData moduleData, size_t argsNumber,
		va_list args);
...
static void caerLearningFilterReset(caerModuleData moduleData,
		uint16_t resetCallSourceID);
static void ModifyForwardSynapse(caerModuleData moduleData,
		int16_t eventSourceID, int64_t preNeuronAddr, int64_t postNeuronAddr,
		double deltaWeight, caerFrameEventPacket *synapseplotfeature,
		caerFrameEventPacket *weightplotfeature);
static void ModifyBackwardSynapse(caerModuleData moduleData,
		int64_t preNeuronAddr, int64_t postNeuronAddr, double deltaWeight,
		caerFrameEventPacket *weightplotfeature);
static bool ResetNetwork(caerModuleData moduleData, int16_t eventSourceID);
static bool BuildSynapse(caerModuleData moduleData, int16_t eventSourceID,
		uint32_t preNeuronAddr, uint32_t postNeuronAddr,
		uint32_t virtualNeuronAddr, int16_t synapseType,
		int8_t realOrVirtualSynapse, int8_t virtualNeuronAddrEnable);
static bool WriteCam(caerModuleData moduleData, int16_t eventSourceID,
		uint32_t preNeuronAddr, uint32_t postNeuronAddr,
		uint32_t virtualNeuronAddr, uint32_t camId, int16_t synapseType,
		int8_t virtualNeuronAddrEnable);
static bool WriteSram(caerModuleData moduleData, int16_t eventSourceID,
		uint32_t preNeuronAddr, uint32_t postNeuronAddr,
		uint32_t virtualNeuronAddr, uint32_t sramId,
		int8_t virtualNeuronAddrEnable);
static void Shuffle1DArray(int64_t *array, int64_t Range);
static void GetRand1DArray(int64_t *array, int64_t Range,
		int64_t CamNumAvailable);
...
static void setBiasBits(caerModuleData moduleData, int16_t eventSourceID,
		uint32_t chipId, uint32_t coreId, const char *biasName_t,
		uint8_t coarseValue, uint16_t fineValue, const char *lowHigh,
		const char *npBias);
static void SetTimer(void);
static void SignalHandler(int m);

COLOUR GetColourW(double v, double vmin, double vmax);
COLOUR GetColourS(int v);

static struct caer_module_functions caerLearningFilterFunctions = {
		.moduleInit = &caerLearningFilterInit, .moduleRun =
				&caerLearningFilterRun, .moduleConfig =
				&caerLearningFilterConfig,
		.moduleExit = &caerLearningFilterExit, .moduleReset =
				&caerLearningFilterReset };

void caerLearningFilter(uint16_t moduleID, int16_t eventSourceID,
		caerSpikeEventPacket spike, caerFrameEventPacket *weightplotfeature,
		caerFrameEventPacket *synapseplotfeature) { //used now
	caerModuleData moduleData = caerMainloopFindModule(moduleID, "LFilter",
			CAER_MODULE_PROCESSOR);
	if (moduleData == NULL) {
		return;
	}
	caerModuleSM(&caerLearningFilterFunctions, moduleData,
			sizeof(struct LFilter_state), 4, eventSourceID, spike,
...
static bool caerLearningFilterInit(caerModuleData moduleData) {
	...
	sshsNodePutDoubleIfAbsent(moduleData->moduleNode, "learningRateBackward",
			2); //5 //2); //1
	sshsNodePutShortIfAbsent(moduleData->moduleNode, "dataSizeX",
			VISUALIZER_HEIGHT_FEATURE); //640
	sshsNodePutShortIfAbsent(moduleData->moduleNode, "dataSizeY",
			VISUALIZER_WIDTH_FEATURE); //480
	sshsNodePutShortIfAbsent(moduleData->moduleNode, "visualizerSizeX",
			VISUALIZER_HEIGHT_FEATURE); //640
	sshsNodePutShortIfAbsent(moduleData->moduleNode, "visualizerSizeY",
			VISUALIZER_WIDTH_FEATURE); //480
	sshsNodePutShortIfAbsent(moduleData->moduleNode, "apsSizeX",
			VISUALIZER_HEIGHT_FEATURE); //640
	sshsNodePutShortIfAbsent(moduleData->moduleNode, "apsSizeY",
			VISUALIZER_WIDTH_FEATURE); //480
	...
	state->visualizerSizeX = sshsNodeGetShort(moduleData->moduleNode,
			"visualizerSizeX");
	state->visualizerSizeY = sshsNodeGetShort(moduleData->moduleNode,
			"visualizerSizeY");
	...
	state->maxSynapseFeature = sshsNodeGetInt(moduleData->moduleNode,
			"maxSynapseFeature");
	state->maxSynapseOutput = sshsNodeGetInt(moduleData->moduleNode,
			"maxSynapseOutput");
...
static void caerLearningFilterConfig(caerModuleData moduleData) {
	...
	LFilterState state = moduleData->moduleState;
	state->colorscaleMax = sshsNodeGetInt(moduleData->moduleNode,
			"colorscaleMax");
	state->colorscaleMin = sshsNodeGetInt(moduleData->moduleNode,
			"colorscaleMin");
	state->reset = sshsNodeGetByte(moduleData->moduleNode, "reset");
	state->resetExProbability = sshsNodeGetFloat(moduleData->moduleNode,
			"resetExProbability");
	state->resetExType = sshsNodeGetByte(moduleData->moduleNode, "resetExType");
	state->resetInType = sshsNodeGetByte(moduleData->moduleNode, "resetInType");
	state->learningRateForward = sshsNodeGetDouble(moduleData->moduleNode,
			"learningRateForward");
	state->learningRateBackward = sshsNodeGetDouble(moduleData->moduleNode,
			"learningRateBackward");
	state->dataSizeX = sshsNodeGetShort(moduleData->moduleNode, "dataSizeX");
	state->dataSizeY = sshsNodeGetShort(moduleData->moduleNode, "dataSizeY");
	state->visualizerSizeX = sshsNodeGetShort(moduleData->moduleNode,
			"visualizerSizeX");
...
static void caerLearningFilterRun(caerModuleData moduleData, size_t argsNumber,
	...
	caerFrameEventPacket *weightplotfeature = va_arg(args,
			caerFrameEventPacket*);
	caerFrameEventPacket *synapseplotfeature = va_arg(args,
			caerFrameEventPacket*);
	...
	if (memory.synapseMap == NULL) {
		...
		if (!ResetNetwork(moduleData, eventSourceID)) { // Failed to allocate memory, nothing to do.
			caerLog(CAER_LOG_ERROR, moduleData->moduleSubSystemString,
					"Failed to allocate memory for synapseMap.");
			return;
		}
		...
		for (i = 0; i < sizeX; i++)
			for (j = 0; j < sizeY; j++)
				warrayS[i][j] = 0;
		*synapseplotfeature = caerFrameEventPacketAllocate(1,
				I16T(moduleData->moduleID), 0, sizeX, sizeY, 3); //put info into frame
		if (*synapseplotfeature != NULL) {
			caerFrameEvent singleplotS = caerFrameEventPacketGetEvent(
					*synapseplotfeature, 0);
			//for feature maps
			for (i = 0; i < INPUT_N; i++) {
				for (j = 0; j < FEATURE1_N * FEATURE1_LAYERS_N; j++) {
					if ((int) (i / INPUT_L)
						...
						row_id = (FILTER1_L + 1 * 2)
								* (int) ((j % FEATURE1_N) / FEATURE1_W)
								+ (int) (i / INPUT_W)
								- (int) ((j % FEATURE1_N) / FEATURE1_W);
						col_id = (FILTER1_W + 1 * 2)
								* ((j % FEATURE1_N) % FEATURE1_W) + i % INPUT_W
								- (j % FEATURE1_N) % FEATURE1_W;
						feature_id = (int) (j / FEATURE1_N);
						warrayS[(feature_id >> 1)
								* (FILTER1_L * FEATURE1_L + 16 * 2) + row_id][(feature_id
								& 0x1) * (FILTER1_W * FEATURE1_W + 16 * 2)
								+ col_id] = memory.synapseMap->buffer2d[((i
								& 0xf) | ((i & 0x10) >> 4) << 8
								| ((i & 0x1e0) >> 5) << 4
								| ((i & 0x200) >> 9) << 9)][j
			...
			for (i = 0; i < sizeX; i++) {
				for (ys = 0; ys < sizeY; ys++) {
					if (i < FILTER1_L * FEATURE1_L + 16 * 2 - 2
							|| ys < FILTER1_W * FEATURE1_W + 16 * 2 - 2) {
			...
			for (i = 0; i < FEATURE1_N * FEATURE1_LAYERS_N; i++) {
				for (j = 0; j < OUTPUT2_N; j++) {
					...
					warrayS[(FILTER1_L + 1 * 2) * FEATURE1_L + row_id][(FILTER1_W
							+ 1 * 2) * FEATURE1_W + col_id] =
							memory.synapseMap->buffer2d[i
									+ TOTAL_NEURON_NUM_ON_CHIP * 2][j
									+ TOTAL_NEURON_NUM_ON_CHIP * 2
			...
			for (i = 0; i < sizeX; i++) {
				for (ys = 0; ys < sizeY; ys++) {
					colS = GetColourS((int) warrayS[ys][i]);
			...
			caerFrameEventSetLengthXLengthYChannelNumber(singleplotS, sizeX,
					sizeY, 3, *synapseplotfeature); //add info to the frame
	...
	if (state->stimulate == true) { //it runs when there is a spike
		if (stimdisabled == 0 && abs(time_count - time_count_last) >= 5) {
			...
			stimdisabled = 1;
		} else if (stimdisabled == 1
				&& abs(time_count - time_count_last) >= 1) {
			if (pattern_number == 3) {
				stimuliPattern = (stimuliPattern + 1) % 3;
			} else if (pattern_number == 4) {
				stimuliPattern = (stimuliPattern + 1) % 4;
			}
			EnableStimuliGen(moduleData, eventSourceID, stimuliPattern + 7);
			time_count_last = time_count;
			stimdisabled = 0;
		}
	} else {
	...
	CAER_SPIKE_ITERATOR_VALID_START(spike) // Iterate over events and update weight
		...
		if (state->learning == true) {
			...
			int64_t ts = caerSpikeEventGetTimestamp64(caerSpikeIteratorElement,
					spike); // Get values on which to operate.

			uint32_t neuronId = caerSpikeEventGetNeuronID(
					caerSpikeIteratorElement);
			uint32_t coreId = caerSpikeEventGetSourceCoreID(
					caerSpikeIteratorElement);
			uint32_t chipId_t = caerSpikeEventGetChipID(
					caerSpikeIteratorElement);
			uint32_t chipId;
			...
			if (memory.wrPointer
					- memory.preRdPointer>= MINIMUM_CONSIDERED_SPIKE_NUM) {

				int64_t preSpikeAddr =
						memory.spikeFifo->buffer2d[memory.preRdPointer][0];
				int64_t preSpikeTime =
						memory.spikeFifo->buffer2d[memory.preRdPointer][1];
				memory.spikeCounter -= 1;
				memory.preRdPointer = (memory.preRdPointer + 1)
						% SPIKE_QUEUE_LENGTH;
				for (uint64_t postRdPointer = (memory.preRdPointer + 1)
						% SPIKE_QUEUE_LENGTH; endSearching != 1;
						postRdPointer =
								(postRdPointer + 1) % SPIKE_QUEUE_LENGTH) {
					int64_t postSpikeAddr =
							memory.spikeFifo->buffer2d[postRdPointer][0];
					int64_t postSpikeTime =
							memory.spikeFifo->buffer2d[postRdPointer][1];
					...
					if (deltaTime < MAXIMUM_CONSIDERED_SPIKE_DELAY) {
						double deltaWeight =
								deltaWeights[DELTA_WEIGHT_LUT_LENGTH - deltaTime];
						if (memory.connectionMap->buffer2d[preSpikeAddr
								- MEMORY_NEURON_ADDR_OFFSET][postSpikeAddr
								- MEMORY_NEURON_ADDR_OFFSET] == 1) {
							ModifyForwardSynapse(moduleData, eventSourceID,
									preSpikeAddr, postSpikeAddr, deltaWeight,
									synapseplotfeature, weightplotfeature);
						}
						if (memory.connectionMap->buffer2d[postSpikeAddr
								- MEMORY_NEURON_ADDR_OFFSET][preSpikeAddr
								- MEMORY_NEURON_ADDR_OFFSET] == 1) {
							ModifyBackwardSynapse(moduleData, preSpikeAddr,
									postSpikeAddr, deltaWeight,
									weightplotfeature);
						}
					}
					...
					if (postRdPointer == memory.wrPointer - 1)
						endSearching = 1;
					else if (deltaTimeAccumulated
							> MAXIMUM_CONSIDERED_SPIKE_DELAY)
						endSearching = 1;
...
void ModifyForwardSynapse(caerModuleData moduleData, int16_t eventSourceID,
		int64_t preSpikeAddr, int64_t postSpikeAddr, double deltaWeight,
	...
	if (memory.connectionMap->buffer2d[preNeuronAddr - MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
		...
		new_weight = memory.weightMap->buffer2d[preNeuronAddr
				- MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
				- MEMORY_NEURON_ADDR_OFFSET]
				+ deltaWeight * state->learningRateForward;

		double increased_weight = 0;
		if (new_weight
				> memory.weightMap->buffer2d[preNeuronAddr
						- MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
						- MEMORY_NEURON_ADDR_OFFSET]) {

			int current_synapse = memory.synapseMap->buffer2d[preNeuronAddr
					- MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
					- MEMORY_NEURON_ADDR_OFFSET];
			synapseUpgrade = 0;

			output_disabled = 0; //memory.outputMapDisabled->buffer2d[preNeuronAddr-MEMORY_NEURON_ADDR_OFFSET][(postNeuronAddr & 0xff)]
			if ((postNeuronAddr & 0x3c00) >> 10 == 3
					&& (postNeuronAddr & 0x300) >> 8 == 3) {
				output_disabled =
						memory.outputMapDisabled->buffer2d[preNeuronAddr
								- MEMORY_NEURON_ADDR_OFFSET][(postNeuronAddr
								& 0xff) / 4];
				if (output_disabled == 0) {
					output_counter = 0;
					for (i = 0; i < OUTPUT2_N; i++) {
						if (memory.outputMap->buffer2d[preNeuronAddr
								- MEMORY_NEURON_ADDR_OFFSET][i] == 1) {
							output_counter += 1;
						}
					}
					if (output_counter >= 2
							|| (output_counter == 1
									&& memory.outputMap->buffer2d[preNeuronAddr
											- MEMORY_NEURON_ADDR_OFFSET][(postNeuronAddr
											& 0xff) / 4] != 1)) {
						output_disabled = 1;
						...
						for (k = 0; k < OUTPUT2_N; k++) {
							...
							for (i = 0; i < OUTPUT2_N; i++) {
								if (memory.outputMap->buffer2d[preNeuronAddr
									...
									for (j = 0; j < n; j++) {
										camId = (uint32_t) j;
										if (memory.camMap->buffer2d[post_neuron_address
											...
											if (neuron_address
												...
												WriteCam(moduleData,
														eventSourceID, 0,
														post_neuron_address, 0,
														(uint32_t) camId, 0, 0);
												memory.camMap->buffer2d[post_neuron_address
														- MEMORY_NEURON_ADDR_OFFSET][camId] =
														0;
												memory.camSize->buffer2d[post_neuron_address
							...
							memory.outputMapDisabled->buffer2d[preNeuronAddr
									- MEMORY_NEURON_ADDR_OFFSET][k] = 1;
							memory.synapseMap->buffer2d[preNeuronAddr
									- MEMORY_NEURON_ADDR_OFFSET][post_neuron_address
									- MEMORY_NEURON_ADDR_OFFSET] = 0;
							if (*synapseplotfeature != NULL) {
								singleplotS = caerFrameEventPacketGetEvent(
										*synapseplotfeature, 0);
							}
							if (*synapseplotfeature != NULL) {
								int64_t preNeuronAddr_t = preNeuronAddr;
								int64_t postNeuronAddr_t = post_neuron_address;
								colS = GetColourS(new_synapse_sub);
								preAddr = (preNeuronAddr_t
										- MEMORY_NEURON_ADDR_OFFSET)
										% TOTAL_NEURON_NUM_ON_CHIP;
								postAddr = (postNeuronAddr_t
										- MEMORY_NEURON_ADDR_OFFSET)
										% TOTAL_NEURON_NUM_ON_CHIP;
								i = preAddr;
								j = postAddr;
								...
								col_id_t = i % 16
										+ ((j % 256) / 4) * (16 + 1 * 2);
								row_id = (FILTER1_L + 1 * 2) * FEATURE1_L
										+ row_id_t;
								col_id = (FILTER1_W + 1 * 2) * FEATURE1_W
										+ col_id_t;
								counterS = (uint32_t) ((row_id * 606) + col_id)
										* 3;
								singleplotS->pixels[counterS] = colS.r;
								singleplotS->pixels[counterS + 1] = colS.g;
								singleplotS->pixels[counterS + 2] = colS.b;
			...
			if (deltaWeight * state->learningRateForward
					> synapseUpgradeThreshold[current_synapse]
					&& (((postSpikeAddr & 0x3c00) >> 10 == 3
							&& (postSpikeAddr & 0x300) >> 8 != 3
							&& current_synapse <= state->maxSynapseFeature)
							|| ((postSpikeAddr & 0x3c00) >> 10 == 3
									&& (postSpikeAddr & 0x300) >> 8 == 3
									&& current_synapse
											<= state->maxSynapseOutput
									&& output_disabled == 0))) {
				...
				if ((postSpikeAddr & 0x3c00) >> 10 == 3
						&& (postSpikeAddr & 0x300) >> 8 != 3) {
					camsize_limit = TOTAL_CAM_NUM_LEARNING;
				} else if ((postSpikeAddr & 0x3c00) >> 10 == 3
						&& (postSpikeAddr & 0x300) >> 8 == 3) {
					if (pattern_number == 3) {
						n = 60;
					} else if (pattern_number == 4) {
				...
				camsize = memory.camSize->buffer2d[postNeuronAddr
						- MEMORY_NEURON_ADDR_OFFSET][0];

				if (camsize < (int) camsize_limit) {
					for (cam_id = 0; cam_id < camsize_limit; cam_id++) { //search for available CAM
						if (memory.camMap->buffer2d[postNeuronAddr
								- MEMORY_NEURON_ADDR_OFFSET][cam_id] == 0) {
							camId = cam_id;
							availableCamFound = 1;
							break;
						}
				...
				if (availableCamFound == 0) {
					for (i = 0; i < camsize_limit; i++) {
						if (memory.camMap->buffer2d[postNeuronAddr
								- MEMORY_NEURON_ADDR_OFFSET][i] != 0) {
							preNeuronId =
									memory.camMapContentSource->buffer2d[postNeuronAddr
											- MEMORY_NEURON_ADDR_OFFSET][i];
							synapseType_t =
									memory.camMapContentType->buffer2d[postNeuronAddr
											- MEMORY_NEURON_ADDR_OFFSET][i];
							if (synapseType_t > 0) { //Real synapse exists
								if (preNeuronId != preNeuronAddr
										&& minFound == 0) {
									minFound = 1;
									min = preNeuronId;
									min_weight =
											memory.weightMap->buffer2d[min
													- MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
													- MEMORY_NEURON_ADDR_OFFSET];
									camId = (uint32_t) i;
									replaced_synapse =
								...
								if (preNeuronId
										== preNeuronAddr&& synapseType_t == EXCITATORY_SLOW_SYNAPSE_ID) { //synapse already exists
									slowFound = 1;
									camId = (uint32_t) i;
									break;
								}
								current_weight_t =
										memory.weightMap->buffer2d[preNeuronId
												- MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
												- MEMORY_NEURON_ADDR_OFFSET];
								if (minFound == 1
										&& preNeuronId != preNeuronAddr
										&& current_weight_t < min_weight) {
									min = preNeuronId;
									min_weight =
											memory.weightMap->buffer2d[min
													- MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
				...
				if (availableCamFound == 1) {
					current_weight = memory.weightMap->buffer2d[preNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET];
					...
					WriteCam(moduleData, eventSourceID,
							(uint32_t) preNeuronAddr, (uint32_t) postNeuronAddr,
							0, camId, synapseType, 0);
					...
					new_synapse_add = current_synapse
							+ EXCITATORY_SLOW_SYNAPSE_ID;
					memory.synapseMap->buffer2d[preNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET] = new_synapse_add;
					memory.connectionCamMap->buffer2d[postNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET][preNeuronAddr
					...
					memory.camMap->buffer2d[postNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET][camId] =
							(int32_t) preNeuronAddr;
					memory.camSize->buffer2d[postNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET][0] += 1;
				}
				...
				if (slowFound == 1) {
					...
					new_synapse_add = current_synapse
							+ EXCITATORY_SLOW_SYNAPSE_ID;
					new_synapse_sub = memory.synapseMap->buffer2d[min
							- MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET] - replaced_synapse;
					if ((postNeuronAddr & 0x3c00) >> 10 == 3
							&& (postNeuronAddr & 0x300) >> 8 == 3) {
						if (new_synapse_sub == 0) {
							memory.outputMap->buffer2d[min
									- MEMORY_NEURON_ADDR_OFFSET][(postNeuronAddr
									& 0xff) / 4] = 0;
						}
					}
					...
					if ((postSpikeAddr & 0x3c00) >> 10 == 3
						...
						if (*synapseplotfeature != NULL) {
							...
							postAddr = (postNeuronAddr_t
									- MEMORY_NEURON_ADDR_OFFSET)
									% TOTAL_NEURON_NUM_ON_CHIP;
							i = (preAddr & 0xf) | ((preAddr & 0x100) >> 8) << 4
									| ((preAddr & 0xf0) >> 4) << 5
									| ((preAddr & 0x200) >> 9) << 9;
							j = postAddr;
							feature_id = (int) (j / FEATURE1_N);
							row_id_t = (FILTER1_L + 1 * 2)
									* (int) ((j % FEATURE1_N) / FEATURE1_W)
									+ (int) (i / INPUT_W)
									- (int) ((j % FEATURE1_N) / FEATURE1_W);
							row_id = (feature_id >> 1)
									* (FILTER1_L * FEATURE1_L + 16 * 2)
									+ row_id_t;
							col_id_t =
									(FILTER1_W + 1 * 2)
											* ((j % FEATURE1_N) % FEATURE1_W)+ i % INPUT_W - (j%FEATURE1_N)%FEATURE1_W;
							col_id = (feature_id & 0x1)
									* (FILTER1_W * FEATURE1_W + 16 * 2)
									+ col_id_t;
				...
				if (synapseUpgrade == 1) {
					if ((postSpikeAddr & 0x3c00) >> 10 == 3
						...
						if (*synapseplotfeature != NULL) {
							...
							int64_t postNeuronAddr_t = postSpikeAddr;
							colS = GetColourS(new_synapse_add);
							preAddr = preNeuronAddr_t
									- MEMORY_NEURON_ADDR_OFFSET;
							postAddr = (postNeuronAddr_t
									- MEMORY_NEURON_ADDR_OFFSET)
...
void ModifyBackwardSynapse(caerModuleData moduleData, int64_t preSpikeAddr,
		int64_t postSpikeAddr, double deltaWeight,
		caerFrameEventPacket *weightplotfeature) {
	...
	if (memory.connectionMap->buffer2d[preNeuronAddr - MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
		...
		new_weight = memory.weightMap->buffer2d[preNeuronAddr
				- MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
				- MEMORY_NEURON_ADDR_OFFSET]
				- deltaWeight * state->learningRateBackward;
...
//	SetInputLayerCam(moduleData, eventSourceID);
	...
	memory.connectionMap = simple2DBufferInitInt(
			(size_t) TOTAL_NEURON_NUM_ON_BOARD,
			(size_t) TOTAL_NEURON_NUM_ON_BOARD);
	memory.filterMap = simple2DBufferInitInt((size_t) TOTAL_NEURON_NUM_ON_BOARD,
			(size_t) MAXIMUM_FILTER_SIZE);
	memory.outputMap = simple2DBufferInitInt((size_t) TOTAL_NEURON_NUM_ON_BOARD,
			(size_t) OUTPUT2_N);
	memory.outputMapDisabled = simple2DBufferInitInt(
			(size_t) TOTAL_NEURON_NUM_ON_BOARD, (size_t) OUTPUT2_N);
	memory.camMapContentSource = simple2DBufferInitInt(
			(size_t) TOTAL_NEURON_NUM_ON_BOARD, (size_t) TOTAL_CAM_NUM);
	memory.camMapContentType = simple2DBufferInitInt(
			(size_t) TOTAL_NEURON_NUM_ON_BOARD, (size_t) TOTAL_CAM_NUM);
	memory.connectionCamMap = simple2DBufferInitInt(
			(size_t) TOTAL_NEURON_NUM_ON_BOARD, (size_t) MAXIMUM_FILTER_SIZE);
	memory.filterMapSize = simple2DBufferInitInt(
			(size_t) TOTAL_NEURON_NUM_ON_BOARD, (size_t) FILTER_MAP_SIZE_WIDTH);

	memory.weightMap = simple2DBufferInitDouble(
			(size_t) TOTAL_NEURON_NUM_ON_BOARD,
			(size_t) TOTAL_NEURON_NUM_ON_BOARD);
	memory.synapseMap = simple2DBufferInitInt(
			(size_t) TOTAL_NEURON_NUM_ON_BOARD,
			(size_t) TOTAL_NEURON_NUM_ON_BOARD);
	memory.camMap = simple2DBufferInitInt((size_t) TOTAL_NEURON_NUM_ON_BOARD,
			(size_t) TOTAL_CAM_NUM);
	memory.camSize = simple2DBufferInitInt((size_t) TOTAL_NEURON_NUM_ON_BOARD,
			(size_t) CAM_SIZE_WIDTH);
	memory.sramMap = simple2DBufferInitInt((size_t) TOTAL_NEURON_NUM_ON_BOARD,
			(size_t) TOTAL_SRAM_NUM);
	memory.sramMapContent = simple2DBufferInitInt(
			(size_t) TOTAL_NEURON_NUM_ON_BOARD, (size_t) TOTAL_SRAM_NUM);
	memory.spikeFifo = simple2DBufferInitLong((size_t) SPIKE_QUEUE_LENGTH,
			(size_t) SPIKE_QUEUE_WIDTH);
	...
	for (neuronId = 0; neuronId < INPUT_N; neuronId++) {
		...
		input_layer[neuronId] = chipId << NEURON_CHIPID_SHIFT
				| ((neuronId & 0xf) | ((neuronId & 0x10) >> 4) << 8
						| ((neuronId & 0x1e0) >> 5) << 4
						| ((neuronId & 0x200) >> 9) << 9);
	}
	...
	for (neuronId = 0; neuronId < OUTPUT2_N; neuronId++) {
		...
		if (neuronId == 0)
			output_layer2[neuronId] = chipId << NEURON_CHIPID_SHIFT
					| coreId << NEURON_COREID_SHIFT | 0;
		if (neuronId == 1)
			output_layer2[neuronId] = chipId << NEURON_CHIPID_SHIFT
					| coreId << NEURON_COREID_SHIFT | 4;
		if (neuronId == 2)
			output_layer2[neuronId] = chipId << NEURON_CHIPID_SHIFT
					| coreId << NEURON_COREID_SHIFT | 8;
		if (neuronId == 3)
			output_layer2[neuronId] = chipId << NEURON_CHIPID_SHIFT
					| coreId << NEURON_COREID_SHIFT | 12;
	}
	...
	//input to feature1
	for (postNeuronId = 0; postNeuronId < FEATURE1_N * FEATURE1_LAYERS_N;
			postNeuronId++) { //FEATURE1_N*FEATURE1_LAYERS_N //first sweep POST, then PRE
		//generate random binary number 1D array
		int64_t rand1DBinaryArray[FILTER1_N]; //FILTER1_N-FEATURE1_CAM_INHIBITORY_N
		GetRand1DBinaryArray(rand1DBinaryArray, FILTER1_N,
				TOTAL_CAM_NUM_LEARNING); //FILTER1_N-FEATURE1_CAM_INHIBITORY_N
		randNumCount = 0;
		for (preNeuronId = 0; preNeuronId < INPUT_N; preNeuronId++) {
			...
			if ((int) (pre_id / INPUT_W)
				...
				if (rand1DBinaryArray[randNumCount] == 1
						&& inhibitoryValid[(postNeuronAddr & 0x300) >> 8][preNeuronAddr
								& 0x3ff] == 0) //build a real synapse
					BuildSynapse(moduleData, eventSourceID, preNeuronAddr,
							postNeuronAddr, virtualNeuronAddr, exType,
							REAL_SYNAPSE, virtualNeuronAddrEnable);
				else if (inhibitoryValid[(postNeuronAddr & 0x300) >> 8][preNeuronAddr
						& 0x3ff] == 0)
					BuildSynapse(moduleData, eventSourceID, preNeuronAddr,
							postNeuronAddr, virtualNeuronAddr, exType,
							VIRTUAL_SYNAPSE, virtualNeuronAddrEnable);
				randNumCount += 1;
			}
		}
	}
	...
	for (preNeuronId = 0; preNeuronId < FEATURE1_N * FEATURE1_LAYERS_N;
			preNeuronId++)
		for (postNeuronId = 0; postNeuronId < FEATURE1_N * FEATURE1_LAYERS_N;
				postNeuronId++) {
			if ((int) (preNeuronId / FEATURE1_N)
					!= (int) (postNeuronId / FEATURE1_N)
					&& (preNeuronId % FEATURE1_N)
							== (postNeuronId % FEATURE1_N)) {
				preNeuronAddr = feature_layer1[preNeuronId];
				postNeuronAddr = feature_layer1[postNeuronId];
				coreId = inhibitoryVirtualNeuronCoreId[(postNeuronAddr & 0x300)
						>> 8][preNeuronAddr & 0xff];
				virtualNeuronAddr = ((preNeuronAddr & 0x3c00) >> 10) << 10
						| coreId << 8 | (preNeuronAddr & 0xff);
				BuildSynapse(moduleData, eventSourceID, preNeuronAddr,
						postNeuronAddr, virtualNeuronAddr,
						(int16_t) (-1 * inType), REAL_SYNAPSE,
	...
	for (postNeuronId = 0; postNeuronId < OUTPUT2_N; postNeuronId++) {
		...
		GetRand1DBinaryArray(rand1DBinaryArray, FEATURE1_N * FEATURE1_LAYERS_N,
				n); //TOTAL_CAM_NUM_LEARNING 60
		randNumCount = 0;
		for (preNeuronId = 0; preNeuronId < FEATURE1_N * FEATURE1_LAYERS_N;
				preNeuronId++) {
			...
			if (rand1DBinaryArray[randNumCount] == 1
					&& (preNeuronAddr & 0x3ff) != 0) {
				BuildSynapse(moduleData, eventSourceID, preNeuronAddr,
						postNeuronAddr, virtualNeuronAddr, exType, REAL_SYNAPSE,
						virtualNeuronAddrEnable);
			} else
				BuildSynapse(moduleData, eventSourceID, preNeuronAddr,
						postNeuronAddr, virtualNeuronAddr, exType,
...
bool BuildSynapse(caerModuleData moduleData, int16_t eventSourceID,
		uint32_t preNeuronAddr, uint32_t postNeuronAddr,
		uint32_t virtualNeuronAddr, int16_t synapseType,
		int8_t realOrVirtualSynapse, int8_t virtualNeuronAddrEnable) {
	...
	if (realOrVirtualSynapse != EXTERNAL_REAL_SYNAPSE) {
		...
		for (sram_id = 0; sram_id < TOTAL_SRAM_NUM; sram_id++) { //search for available SRAM
			...
			if (memory.sramMap->buffer2d[preNeuronAddr
					- MEMORY_NEURON_ADDR_OFFSET][sram_id] == 1
					&& memory.sramMapContent->buffer2d[preNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET][sram_id]
							== chipCoreId) { //start the searching from second SRAM, for visualization
				sramFound = 1;
			}
		}
		if (sramFound == 0) {
			...
			for (sram_id = 0; sram_id < TOTAL_SRAM_NUM; sram_id++) { //search for available SRAM
				if (sramAvailable == 0
						&& memory.sramMap->buffer2d[preNeuronAddr
								- MEMORY_NEURON_ADDR_OFFSET][(sram_id + 1)
								% TOTAL_SRAM_NUM] == 0) {
					sramAvailable = 1;
			...
			if (sramAvailable == 1 && sramId != 0) { //sramId != 0 && sramId != 1 && sramId != 2 && sramId != 3
				WriteSram(moduleData, eventSourceID, preNeuronAddr,
						postNeuronAddr, virtualNeuronAddr, sramId,
						virtualNeuronAddrEnable);
				memory.sramMap->buffer2d[preNeuronAddr
						- MEMORY_NEURON_ADDR_OFFSET][sramId] = 1; //taken
				chipCoreId = (int) (postNeuronAddr >> 8);
				memory.sramMapContent->buffer2d[preNeuronAddr
						- MEMORY_NEURON_ADDR_OFFSET][sramId] = chipCoreId; //taken
			}
		}
	}
	//for CAM
	...
	for (cam_id = 0; cam_id < TOTAL_CAM_NUM; cam_id++) { //search for available CAM
		if (synapseType > 0) {
			if (memory.camMapContentSource->buffer2d[postNeuronAddr
					- MEMORY_NEURON_ADDR_OFFSET][cam_id]
					== (int32_t) preNeuronAddr) {
				camFound = 1;
				break;
			}
			...
			if (memory.camMapContentSource->buffer2d[postNeuronAddr
					...
					|| (memory.camMapContentSource->buffer2d[postNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET][cam_id]
							== (int32_t) virtualNeuronAddr
							&& virtualNeuronAddrEnable == 1)) {
				camFound = 1;
				break;
			}
		}
		if (memory.camMap->buffer2d[postNeuronAddr - MEMORY_NEURON_ADDR_OFFSET][cam_id]
				== 0) {
			camId = cam_id;
			break;
		}
	...
	if (realOrVirtualSynapse == REAL_SYNAPSE
			|| realOrVirtualSynapse == EXTERNAL_REAL_SYNAPSE) {
		if (camFound == 0) {
			output_disabled = 0;
			if ((postNeuronAddr & 0x3c00) >> 10 == 3
					&& (postNeuronAddr & 0x300) >> 8 == 3) {
				...
				if (output_disabled == 0) {
					WriteCam(moduleData, eventSourceID, preNeuronAddr,
							postNeuronAddr, virtualNeuronAddr, camId,
							synapseType, virtualNeuronAddrEnable);
					memory.outputMap->buffer2d[preNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr & 0xff] =
							1;
				}
			} else {
				WriteCam(moduleData, eventSourceID, preNeuronAddr,
						postNeuronAddr, virtualNeuronAddr, camId, synapseType,
						virtualNeuronAddrEnable);
			}
			WriteCam(moduleData, eventSourceID, preNeuronAddr, postNeuronAddr,
					virtualNeuronAddr, camId, synapseType,
					virtualNeuronAddrEnable);
	...
	if (synapseType > 0) { //if it is EX synapse
		int32_t memoryId = memory.filterMapSize->buffer2d[postNeuronAddr
				- MEMORY_NEURON_ADDR_OFFSET][0];
		memory.filterMap->buffer2d[postNeuronAddr - MEMORY_NEURON_ADDR_OFFSET][memoryId] =
				(int32_t) preNeuronAddr;
		memory.connectionCamMap->buffer2d[postNeuronAddr
				- MEMORY_NEURON_ADDR_OFFSET][memoryId] = (int32_t) camId;
		memory.filterMapSize->buffer2d[postNeuronAddr
				- MEMORY_NEURON_ADDR_OFFSET][0] += 1;
		if (realOrVirtualSynapse != EXTERNAL_REAL_SYNAPSE) {
			memory.connectionMap->buffer2d[preNeuronAddr
					- MEMORY_NEURON_ADDR_OFFSET][postNeuronAddr
			...
			if (realOrVirtualSynapse == REAL_SYNAPSE) {
				...
				if (camFound == 0) {
					memory.camMapContentType->buffer2d[postNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET][camId] = synapseType;
					memory.camMapContentSource->buffer2d[postNeuronAddr
							- MEMORY_NEURON_ADDR_OFFSET][camId] =
							(int32_t) preNeuronAddr;
					if ((postNeuronAddr & 0x3c00) >> 10 == 3
		...
		memory.camMapContentType->buffer2d[postNeuronAddr
				- MEMORY_NEURON_ADDR_OFFSET][camId] = synapseType;
		memory.camMapContentSource->buffer2d[postNeuronAddr
				- MEMORY_NEURON_ADDR_OFFSET][camId] = (int32_t) preNeuronAddr;
	}
	return (true);
}

//write neuron CAM when a synapse is built or modified
bool WriteCam(caerModuleData moduleData, int16_t eventSourceID,
		uint32_t preNeuronAddr, uint32_t postNeuronAddr,
		uint32_t virtualNeuronAddr, uint32_t camId, int16_t synapseType,
		int8_t virtualNeuronAddrEnable) {
	...
	state->eventSourceModuleState = caerMainloopGetSourceState(
			U16T(eventSourceID));
	state->eventSourceConfigNode = caerMainloopGetSourceNode(
			U16T(eventSourceID));
	...
	if (virtualNeuronAddrEnable == 0)
		source_core = (preNeuronAddr & NEURON_COREID_BITS)
				>> NEURON_COREID_SHIFT;
	else
		source_core = (virtualNeuronAddr & NEURON_COREID_BITS)
				>> NEURON_COREID_SHIFT; //to change
	if (synapseType > 0) //if it is EX synapse
	...
	uint32_t coreId = (postNeuronAddr & NEURON_COREID_BITS)
			>> NEURON_COREID_SHIFT;
	...
	bits = ei << CXQ_CAM_EI_SHIFT | fs << CXQ_CAM_FS_SHIFT
			...
			CXQ_PROGRAM | coreId << CXQ_PROGRAM_COREID_SHIFT
			| row << CXQ_PROGRAM_ROW_SHIFT | column << CXQ_PROGRAM_COLUMN_SHIFT;
	caerDeviceConfigSet(stateSource->deviceState, DYNAPSE_CONFIG_CHIP,
			DYNAPSE_CONFIG_CHIP_ID, chipId);
	caerDeviceConfigSet(stateSource->deviceState, DYNAPSE_CONFIG_CHIP,
			DYNAPSE_CONFIG_CHIP_CONTENT, bits); //this is the 30 bits

	return (true);
}

//write neuron SRAM when a synapse is built
bool WriteSram(caerModuleData moduleData, int16_t eventSourceID,
		uint32_t preNeuronAddr, uint32_t postNeuronAddr,
		uint32_t virtualNeuronAddr, uint32_t sramId,
		int8_t virtualNeuronAddrEnable) {
//	caerDeviceHandle usb_handle = ((caerInputDynapseState) moduleData->moduleState)->deviceState; doesn't work
	...
	if (virtualNeuronAddrEnable == 0)
		virtual_coreId = (preNeuronAddr & NEURON_COREID_BITS)
				>> NEURON_COREID_SHIFT;
	else
		virtual_coreId = (virtualNeuronAddr & NEURON_COREID_BITS)
				>> NEURON_COREID_SHIFT;
	...
	uint32_t dest_coreId = (uint32_t) (1
			<< ((postNeuronAddr & NEURON_COREID_BITS) >> NEURON_COREID_SHIFT));
	uint32_t coreId = (preNeuronAddr & NEURON_COREID_BITS)
			>> NEURON_COREID_SHIFT;
	...
	uint32_t row = neuron_row << SRAM_NEURON_ROW_SHIFT
			| neuron_column << SRAM_NEURON_COL_SHIFT | synapse_row;
	uint32_t column = SRAM_COL_VALUE;
	bits = virtual_coreId << CXQ_SRAM_VIRTUAL_SOURCE_CORE_SHIFT
			| sy << CXQ_SRAM_SY_SHIFT | dy << CXQ_SRAM_DY_SHIFT
			| sx << CXQ_SRAM_SX_SHIFT | dx << CXQ_SRAM_DX_SHIFT
...
bool ClearAllCam(caerModuleData moduleData, int16_t eventSourceID) {
	...
	for (neuronId = 0; neuronId < 32 * 32; neuronId++) {
		for (camId = 0; camId < 64; camId++) {
			WriteCam(moduleData, eventSourceID, 0, 1 << 10 | neuronId, 0, camId,
					0, 0);
			WriteCam(moduleData, eventSourceID, 0, 2 << 10 | neuronId, 0, camId,
					0, 0);
			WriteCam(moduleData, eventSourceID, 0, 3 << 10 | neuronId, 0, camId,
					0, 0);
			WriteCam(moduleData, eventSourceID, 0, 4 << 10 | neuronId, 0, camId,
					0, 0);
		}
	}
	return (true);
...
bool EnableStimuliGen(caerModuleData moduleData, int16_t eventSourceID,
	...
	sshsNode deviceConfigNodeMain = sshsGetRelativeNode(
...
bool SetInputLayerCam(caerModuleData moduleData, int16_t eventSourceID) {
	...
	for (rowId = 0; rowId < DYNAPSE_CONFIG_XCHIPSIZE; rowId++) {
		for (colId = 0; colId < DYNAPSE_CONFIG_YCHIPSIZE; colId++) {
			neuronId = (uint32_t) (1 << 10 | ((rowId & 0X10) >> 4) << 9
					| ((colId & 0X10) >> 4) << 8 | (rowId & 0xf) << 4
					| (colId & 0xf));

			if (pattern_number == 3) {
				if (spikePatternA[rowId][colId] == 1)
					WriteCam(moduleData, eventSourceID, 1, neuronId, 0, 0, 2,
							0);
				if (spikePatternB[rowId][colId] == 1)
					WriteCam(moduleData, eventSourceID, 2, neuronId, 0, 1, 2,
							0);
				if (spikePatternC[rowId][colId] == 1)
					WriteCam(moduleData, eventSourceID, 3, neuronId, 0, 2, 2,
							0);
			} else if (pattern_number == 4) {
				...
				if (spikePatternB[rowId][colId] == 1 && rowId >= 16)
					WriteCam(moduleData, eventSourceID, 3, neuronId, 0, 2, 2,
							0);
				if (spikePatternB[rowId][colId] == 1 && colId >= 16)
					WriteCam(moduleData, eventSourceID, 4, neuronId, 0, 3, 2,
							0);
			}
		}
	}
	...
	for (chipId_t = 0; chipId_t < 4; chipId_t++) { //1 4
		...
		for (coreId = 0; coreId < 4; coreId++) {
			...
			if (chipId == DYNAPSE_CONFIG_DYNAPSE_U0) {
				if (coreId == 0) {
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_AHTAU_N", 7, 35, "LowBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_AHTHR_N", 7, 0, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_AHW_P", 7, 0, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_BUF_P", 3, 80, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_CASC_N", 7, 0, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_DC_P", 7, 0, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_NMDA_N", 7, 0, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_RFR_N", 6, 255, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_TAU1_N", 4, 200, "LowBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_TAU2_N", 6, 15, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_THR_N", 3, 100, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_TAU_F_P", 6, 105, "HighBias", "PBias"); //105
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_TAU_S_P", 7, 40, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_THR_F_P", 0, 220, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_THR_S_P", 7, 0, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_TAU_F_P", 7, 40, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_TAU_S_P", 7, 40, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_THR_F_P", 7, 40, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_THR_S_P", 7, 40, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_EXC_F_N", 0, 76, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_EXC_S_N", 7, 0, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_INH_F_N", 7, 0, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_INH_S_N", 7, 0, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PULSE_PWLK_P", 3, 50, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"R2R_P", 4, 85, "HighBias", "PBias");
				}
				if (coreId == 1) {
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_AHTAU_N", 7, 35, "LowBias", "NBias");
					...
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_TAU2_N", 6, 15, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_THR_N", 4, 40, "HighBias", "NBias"); //2, 40
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_TAU_F_P", 6, 105, "HighBias", "PBias"); //105
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_TAU_S_P", 7, 40, "HighBias", "NBias");
				...
				if (coreId == 0) {
					...
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_NMDA_N", 7, 0, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_RFR_N", 5, 103, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_TAU1_N", 4, 200, "LowBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_TAU2_N", 6, 15, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_THR_N", 0, 200, "HighBias", "NBias"); //3, 150 //4, 40
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_TAU_F_P", 7, 30, "HighBias", "PBias"); //70!!!!! //6, 200 //105
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_TAU_S_P", 6, 200, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_THR_F_P", 0, 220, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_THR_S_P", 0, 220, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_TAU_F_P", 7, 30, "HighBias", "PBias"); //7, 40, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_TAU_S_P", 7, 40, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_THR_F_P", 0, 150, "HighBias", "PBias"); //7, 40, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_THR_S_P", 7, 40, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_EXC_F_N", 1, 250, "HighBias", "NBias"); //29!!!
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_EXC_S_N", 1, 100, "HighBias", "NBias"); //19!!! //0, 38
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_INH_F_N", 0, 250, "HighBias", "NBias"); //7, 0, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_INH_S_N", 7, 0, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PULSE_PWLK_P", 3, 50, "HighBias", "PBias");
					...
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_TAU2_N", 6, 15, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_THR_N", 1, 150, "HighBias", "NBias"); //100!!!!! //17!!! //3, 150 //4, 40
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_TAU_F_P", 7, 40, "HighBias", "PBias"); //6, 200 //105
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_TAU_S_P", 6, 200, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_THR_F_P", 0, 250, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_THR_S_P", 0, 220, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_TAU_F_P", 7, 30, "HighBias", "PBias"); //7, 40, "HighBias", "NBias");
					...
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_THR_S_P", 7, 40, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_EXC_F_N", 0, 250, "HighBias", "NBias"); //28!!!
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_EXC_S_N", 0, 100, "HighBias", "NBias"); //19!!! //0, 38
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_INH_F_N", 0, 250, "HighBias", "NBias"); //7, 0, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_INH_S_N", 7, 0, "HighBias", "NBias");
					...
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_RFR_N", 5, 103, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_TAU1_N", 4, 245, "LowBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_TAU2_N", 6, 15, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_THR_N", 1, 100, "HighBias", "NBias"); //10!!! //3, 150 //4, 40
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_TAU_F_P", 7, 60, "HighBias", "PBias"); //6, 200 //105
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_TAU_S_P", 6, 200, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_THR_F_P", 0, 250, "HighBias", "PBias"); //220!!!
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_THR_S_P", 0, 220, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_TAU_F_P", 7, 50, "HighBias", "PBias"); //7, 40, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_TAU_S_P", 7, 40, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_THR_F_P", 0, 150, "HighBias", "PBias"); //7, 40, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_THR_S_P", 7, 40, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_EXC_F_N", 0, 250, "HighBias", "NBias"); //237!!!
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_EXC_S_N", 0, 150, "HighBias", "NBias"); //0, 38
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_INH_F_N", 0, 250, "HighBias", "NBias"); //7, 0, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_INH_S_N", 7, 0, "HighBias", "NBias");
					...
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_TAU2_N", 6, 15, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"IF_THR_N", 1, 45, "HighBias", "NBias"); //3, 150 //4, 40
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_TAU_F_P", 6, 100, "HighBias", "PBias"); //6, 200 //105
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_TAU_S_P", 6, 200, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_THR_F_P", 0, 220, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPIE_THR_S_P", 0, 220, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_TAU_F_P", 6, 105, "HighBias", "PBias"); //7, 40, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_TAU_S_P", 7, 40, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_THR_F_P", 0, 250, "HighBias", "PBias"); //7, 40, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"NPDPII_THR_S_P", 7, 40, "HighBias", "PBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_EXC_F_N", 0, 76, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_EXC_S_N", 0, 19, "HighBias", "NBias"); //0, 38
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_INH_F_N", 0, 250, "HighBias", "NBias"); //7, 0, "HighBias", "NBias");
					setBiasBits(moduleData, eventSourceID, chipId, coreId,
							"PS_WEIGHT_INH_S_N", 7, 0, "HighBias", "NBias");
				...
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"IF_TAU2_N", 6, 15, "HighBias", "NBias");
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"IF_THR_N", 5, 40, "HighBias", "NBias");
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"NPDPIE_TAU_F_P", 6, 105, "HighBias", "PBias");
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"NPDPIE_TAU_S_P", 7, 40, "HighBias", "NBias");
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"NPDPIE_THR_F_P", 0, 220, "HighBias", "PBias");
				...
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"NPDPII_TAU_S_P", 7, 40, "HighBias", "NBias");
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"NPDPII_THR_F_P", 0, 220, "HighBias", "PBias"); //7, 40, "HighBias", "PBias");
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"NPDPII_THR_S_P", 7, 40, "HighBias", "PBias");
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"PS_WEIGHT_EXC_F_N", 0, 76, "HighBias", "NBias");
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"PS_WEIGHT_EXC_S_N", 7, 0, "HighBias", "NBias");
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"PS_WEIGHT_INH_F_N", 0, 76, "HighBias", "NBias"); //7, 0, "HighBias", "NBias");
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"PS_WEIGHT_INH_S_N", 7, 0, "HighBias", "NBias");
				setBiasBits(moduleData, eventSourceID, chipId, coreId,
						"PULSE_PWLK_P", 3, 50, "HighBias", "PBias");
				setBiasBits(moduleData, eventSourceID, chipId, coreId, "R2R_P",
						4, 85, "HighBias", "PBias");
			}
...
void setBiasBits(caerModuleData moduleData, int16_t eventSourceID,
		uint32_t chipId, uint32_t coreId, const char *biasName_t,
		uint8_t coarseValue, uint16_t fineValue, const char *lowHigh,
		const char *npBias) {
	LFilterState state = moduleData->moduleState;
